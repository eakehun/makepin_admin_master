	...
	<div>
		...
		<table class="table table-no-more">
			<tbody>
				<tr>
					<th>제목</th>
					<td colspan="3"><b-form-input placeholder="" v-model="userWritingData.title"></b-form-input></td>
				</tr>
				<tr>
					<th>작성일시</th>
					<td>-</td>
				...
				<tr>
					...
					<td colspan="3">
						<b-form-group v-slot="{ ariaDescribedby }" class="radio_box">
							<b-form-radio v-model="userWritingData.fixed" :aria-describedby="ariaDescribedby" name="some-radios" value="true">고정</b-form-radio>
							<b-form-radio v-model="userWritingData.fixed" :aria-describedby="ariaDescribedby" name="some-radios" value="false">일반</b-form-radio>
						</b-form-group>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<b-form-group v-slot="{ ariaDescribedby }" class="radio_box">
							<b-form-radio v-model="userWritingData.display" :aria-describedby="ariaDescribedby" name="some-radios2" value="true">노출</b-form-radio>
							<b-form-radio v-model="userWritingData.display" :aria-describedby="ariaDescribedby" name="some-radios2" value="false">미노출</b-form-radio>
						</b-form-group>
					</td>
				</tr>
				<tr>
					<th>내용</th>
					<td colspan="3">
						<ckeditor :editor="editor" v-model="userWritingData.content" :config="editorConfig" @ready="onEditorReady"></ckeditor>
					</td>
				</tr>
			</tbody>
		</table>
		<div class="btn_wrap first_btn_cont">
			<b-button variant="primary" class="historyBack" @click="noticeUpload">등록하기</b-button>
...
<script>
	...
	export default{
		...
		methods: {
			...
			noticeUpload(){
				if(this.userWritingData.title == '' || this.userWritingData.content == ''){
					this.showMsgBoxOne('모든 항목은 필수 입력 항목입니다.')
				}
				if(this.userWritingData.content != ''){
					this.showMsgBoxTwo()
				}
			},
			...
			uploadData(){
				axios
				.post(process.env.VUE_APP_BASE_URL+'/admin/board/notice/', this.userWritingData, { withCredentials: true })
				.then(function (response) {
					this.$router.push({name: 'noticeBoard', path: '/site/noticeBoard', query: {idx: response.data.idx}});
				}.bind(this))
