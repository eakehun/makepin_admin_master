	...
	<div>
		...
		<div v-if="detailSta == false">
			<div>
				<table class="table b-table table_typeA">
					...
					<tbody role="rowgroup">
						...
						<tr role="row" class="">
							<td role="cell" class="">고정여부</td>
							<td role="cell" class="">
								<b-form-group v-slot="{ ariaDescribedby }" class="radio_box">
									<b-form-radio v-model="fixedSelected" :aria-describedby="ariaDescribedby" name="some-radios2" value="전체">전체</b-form-radio>
									<b-form-radio v-model="fixedSelected" :aria-describedby="ariaDescribedby" name="some-radios2" value="true">고정</b-form-radio>
									<b-form-radio v-model="fixedSelected" :aria-describedby="ariaDescribedby" name="some-radios2" value="false">일반</b-form-radio>
								</b-form-group>
							</td>
						</tr>
			...
			<div class="of_cneter">
				...
				<b-button variant="outline-dark" class="f_right" size="sm" to="/site/noticeBoardWriting">등록하기</b-button>
			</div>
			...
			<div>
				<b-table responsive :items="items" :fields="fields">
					...
					<template #cell(title)="data">
						<div class="detail_go_btn" @click="detailGo(data.item.idx)">{{data.item.idx}} {{ data.item.title }}</div>
					</template>
			...
			<table class="table table-no-more">
				<tbody>
					...
					<tr>
						...
						<td colspan="3">
							<b-form-group v-slot="{ ariaDescribedby }" class="radio_box">
								<b-form-radio v-model="detailData.fixed" :aria-describedby="ariaDescribedby" disabled="disabled" name="some-radios" value="true">고정</b-form-radio>
								<b-form-radio v-model="detailData.fixed" :aria-describedby="ariaDescribedby" disabled="disabled" name="some-radios" value="false">일반</b-form-radio>
							</b-form-group>
						</td>
					</tr>
					<tr>
	...
	export default{
		...
		methods: {
			...
			userSearchBtn(){
				...
				// console.log(this.displayName+this.fixedUrlName+this.searchUrlName1+this.searchUrlName2)
				axios
				.get(process.env.VUE_APP_BASE_URL + '/admin/board/notice/fromDate/'+this.datepickerValue1+' 00:00:00/toDate/'+this.datepickerValue2+' 23:58:58/'+this.displayName+this.fixedUrlName+this.searchUrlName1+this.searchUrlName2+'?page=0&size=10&sort=idx,desc', { withCredentials: true })
				.then( res => {
			...
			detailModify(idxNum){
				this.$router.push({name: 'noticeBoardModify', path: '/site/noticeBoardModify', query: {idx: idxNum}});
			},

			detailGo(idx){
				this.detailSta = true
				axios
				.get(process.env.VUE_APP_BASE_URL + '/admin/board/notice/idx/'+idx+'/', { withCredentials: true })
				.then( res => {
			...
			detailClose(){
				if(this.$route.query.idx != undefined){
					this.$router.push({name: 'noticeBoard', path: '/site/noticeBoard', query: ''});
				}
			...
			resetSearchBtn(){
				...
				this.totalPages = 1
				this.currentPage = 1
				this.$router.push({name: 'noticeBoard', path: '/site/noticeBoard'}, function() {
					// console.log("noticeBoard 호출 완료");
				});
			},
		...
		watch: {
			...
			fixedSelected(data){
				if(data == '전체'){
					return this.fixedUrlName = ''
				}
				this.fixedUrlName = 'fixed/'+data+'/'
			},
			...
			$route (data){
				...
				axios
				.get(process.env.VUE_APP_BASE_URL + '/admin/board/notice/fromDate/'+this.datepickerValue1+' 00:00:00/toDate/'+this.datepickerValue2+' 23:58:58/'+this.displayName+this.fixedUrlName+this.searchUrlName1+this.searchUrlName2+'?page='+this.$route.query.page+'&size=10&sort=idx,desc', { withCredentials: true })
				.then( res => {
					this.totalElements = res.data.totalElements
					this.items = res.data.content
					this.totalPages = res.data.totalPages
